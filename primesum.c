...
void* producer(void* arg) {
    ...
    pthread_mutex_lock(&array_mutex);
    //Only insert the sentinel if the slot is empty to avoid overwriting a prime
    while (prime_array[id] != SLOT_EMPTY) {
        pthread_cond_wait(&array_not_full[id], &array_mutex);
    }
    prime_array[id] = -1;  
    //Sentinel value indicating producer is done
    pthread_cond_signal(&array_not_empty);  
    //Wake consumer to check for done signal
    pthread_mutex_unlock(&array_mutex);
...
void* consumer(void* arg) {
    ...
    int finished[MAX_THREADS] = {0};  
    //Track if a producer is already counted as done
    ...
    while (1) {
        ...
        for (int i = 0; i < MAX_THREADS; i++) {
            if (prime_array[i] > 0) {
                //Add a printf for debugging to see each prime being added
                //printf("Adding prime: %d\n", prime_array[i]);
                sum += prime_array[i];
                prime_array[i] = SLOT_EMPTY;
                pthread_cond_signal(&array_not_full[i]);  
                //Let the producer know the slot is free
            } else if (prime_array[i] == -1 && !finished[i]) {
                finished[i] = 1;
                prime_array[i] = SLOT_EMPTY;
                producers_finished++;
                pthread_cond_signal(&array_not_full[i]);  
                //Signal the producer for sentinel values
            }
        }
...
int main(int argc, char* argv[]) {
    ...
    //Join all producer threads to ensure they complete before consumer finishes
    for (int i = 0; i < MAX_THREADS; i++) {
